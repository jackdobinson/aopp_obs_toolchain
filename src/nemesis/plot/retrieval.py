...
To operate on multiple directories at once use the following BASH incantation:
```
CWD=$PWD
for ADIR in $PWD/*/; do
	cd $ADIR
	~/Documents/code/python3/standalone/nemesis/plot/retrieval.py --plot all --ensemble_plot all
done
cd $CWD
``` 
"""
...
def spx(runname, outfolder=None, show_plot=False, **kwargs):
	"""
	Plots the data present in a *.spx file
	"""
	ut.pINFO('Plotting spectra from *.spx file')
	try:
		spxd = nemesis.read.spx(runname)
	except nemesis.exceptions.NemesisReadError:
		ut.pWARN(f'Could not read *.spx file from {runname}. Skipping...')
	f1 = plt.figure(figsize=(8,8))
	a1 = f1.subplots(2,1, squeeze=False)
	for i in range(spxd['ngeom']):
		a1[0,0].errorbar(spxd['spec_record'][i][:,0], spxd['spec_record'][i][:,1], yerr=spxd['spec_record'][i][:,2], 
						   label=f'ngeom {i}', linewidth=0.5, capthick=1, capsize=2)
	a1[0,0].set_xlabel('Wavelength (um)')
	a1[0,0].set_ylabel('Radiance')
	a1[0,0].legend()
	for i in range(spxd['ngeom']):
		a1[1,0].errorbar(spxd['spec_record'][i][:,0], spxd['spec_record'][i][:,1], yerr=spxd['spec_record'][i][:,2], 
						   label=f'ngeom {i}', linewidth=0.5, capthick=1, capsize=2)
	a1[1,0].set_xlabel('Wavelength (um)')
	a1[1,0].set_ylabel('Radiance')
	a1[1,0].set_yscale('log')
	a1[1,0].legend()
	f1.suptitle('spx file geometries')
	save_show_plt(f1, "spx.png", outfolder=outfolder, show_plot=show_plot)
	return([f1])

def mre_diff(runname, outfolder=None, show_plot=False, **kwargs):
	"""
	Creates plots that highlight the differences between different geometries
	"""
	ut.pINFO('Plotting differences in spectral data from *.mre file...')
	
	try:
		mre_data= nemesis.read.mre(runname)
	except nemesis.exceptions.NemesisReadError as nre:
		print(nre)
		ut.pWARN(f'Could not read *.mre file from {runname}, skipping...')
		return([None])
	...
	wl, rm, rr, re, rpd = (mre_data['waveln'], mre_data['radiance_meas'], mre_data['radiance_retr'], mre_data['radiance_err'], mre_data['radiance_perc_diff'])
	for i in range(1,ngeom):
		rm_diff_i = rm[0] - rm[i]
		rr_diff_i = rr[0] - rr[i]
		re_diff_i = re[0] + re[i]
		
		plot_spec_err(np.array([wl[i]]), np.array([rm_diff_i]), e=np.array([re_diff_i]), fig=f1, ax=a1[i-1,0], ylabel='Radiance difference\nuW cm^{-2} sr^{-1} cm', data_label=[f'a-priori {0} - {i}'])
		a1[i-1,0].set_title('apriori radiance differences')
		plot_spec_err(np.array([wl[i]]), np.array([rr_diff_i]), e=None, fig=f1, ax=a1[i-1,1], ylabel='', data_label=[f'retrieved {0} - {i}'])
		a1[i-1,1].set_title('retrieved radiance differenes')
		# set limits to be the same for apriori and retrieved differences
		ylims_0 = a1[i-1,0].get_ylim()
		ylims_1 = a1[i-1,1].get_ylim()
		ymin = ylims_0[0] if ylims_0[0] < ylims_1[0] else ylims_1[0]
		ymax = ylims_0[1] if ylims_0[1] > ylims_1[1] else ylims_1[1]
		a1[i-1,0].set_ylim((ymin,ymax))
		a1[i-1,1].set_ylim((ymin,ymax))

	f1.suptitle('Geometry Differences')
	save_show_plt(f1, f'mre_diff.png', outfolder=outfolder, show_plot=show_plot)
	return([f1])
...
def mre_spectral(runname, outfolder=None, show_plot=False, log_scale=True, **kwargs):
	...
	y_min = np.inf
	y_max = -np.inf
	for i, (rm, rr) in enumerate(zip(mre_data['radiance_meas'], mre_data['radiance_retr'])):
		if np.min(rm) < y_min:
			y_min = np.min(rm)
		if np.min(rr) < y_min:
			y_min = np.min(rr)
	...
	for i, (wl, rm, rr, re, rpd) in enumerate(zip(	mre_data['waveln'], 
		...
		fig1 = plt.figure(figsize=[x/2.54 for x in (nx*16,ny*12+2)])
		# create spectral plot
		a11 = fig1.add_subplot(ny,nx,1)
		a12 = fig1.add_subplot(ny,nx,2)
		"""
		...
		a11.set_ylim((y_min,y_max))
		#else:
		plot_spec_err(np.array([wl,wl]), np.array([rm,rr]), e=[re,None], fig=fig1, ax=a12, ylabel='Radiance\nuW cm^{-2} sr^{-1} cm', data_label=['Measurement', 'Retrieved'])
		a12.set_ylim((y_min,y_max))

		a21 = fig1.add_subplot(ny,nx,3)
		a22 = a21.twinx()
...
def chi_sq(runname, outfolder=None, show_plot=False, **kwargs):
	...
	except (nemesis.exceptions.NemesisReadError, FileNotFoundError) as e:
...
def ensemble_minnaert_limb_darkening(runnames, outfolder=None, show_plot=False, **kwargs):
	...
	"""
	# DEBUGGING
	ut.pWARN('exiting from "ensemble_minnaert_limb_darkening" as it does not work correctly')
	return([])
	# END DEBUGGING
	print('INFO: In "ensemble_minnaert_limb_darkening()"')
	...
	# loop over runs, we should have one for each latitude band
	for i, runname in enumerate(runnames):
		...
		fov_array = np.array(spx_data['fov_averaging_record'])
		#print(fov_array)
		#print(fov_array.shape)
	
		# I'm not taking different FOVs into account for a given ngeom. Probably ok as don't need that feature much.	
		u = np.cos(np.ones_like(mre_data['waveln'])*fov_array[:,0,3,None]*np.pi/180) # emission angle (observer zenith) 
		u0 = np.cos(np.ones_like(mre_data['waveln'])*fov_array[:,0,2,None]*np.pi/180) # incident angle (solar zenith)
		...
		# calculate minnaert limb darkening parameters for apriori and retrieved spectra
		...
		# each mld has these values for each wavelength (x, residual, rank, s, log_u0u, log_uIperF, nandata)
		# x = [IperF0, k], limn darkening parameters
		# residual = fitted value - minnaert calculated value
		# rank = rank of coefficent matrix
		# s = singlular values of coefficient matrix
		# log_u0u = log(u * u0)
		# log_uIperF = log(u * (I/F))
		# nandata = indices where log_uIperF equals NAN
		...
		# get initial and retrieved limb darkening parameters
		...
		# get latitude range from file path, if we didn't do this we would have to
		# create an ancilliary file that holds this info when we create the the nemesis inputs
	...
	##############################################################################
	### Create plot of wavelength vs ln((I/F)_0) and k for each LATITUDE slice ###
	print('INFO: Creating minnaert params plot vs latitude...')
	...
	print(ordered_idxs)
	ani = matplotlib.animation.FuncAnimation(f1, update, ordered_idxs, interval=1000)
	outmovname = 'minnaert_params_v_lat.mov'
	print(outfolder)
	...
	################################################################################
	### Create plot of wavelength vs ln((I/F)_0) and k for each WAVELENGTH slice ###
	print('INFO: Creating minnaert params plot vs wavelength...')
	f1 = plt.figure(figsize=[_x/2.54 for _x in (12*2,12*2)])

	print('lat_ranges', lat_ranges)
	mean_lats = np.mean(lat_ranges, axis=1)
	print('mean_lats', mean_lats)
	# wavelengths
	xmin = np.nanmin(np.concatenate((initial_mlds[0,0,:], retrieved_mlds[0,0,:]), axis=None))
	xmax = np.nanmax(np.concatenate((initial_mlds[0,0,:], retrieved_mlds[0,0,:]), axis=None))
	print(xmin)
	...
	ln_IperF0_min = np.nanmin(np.concatenate((initial_mlds[:,1,:].flatten(), retrieved_mlds[:,1,:].flatten()), axis=None))
	ln_IperF0_max = np.nanmax(np.concatenate((initial_mlds[:,1,:].flatten(), retrieved_mlds[:,1,:].flatten()), axis=None))

	#k_min = np.nanmin(np.concatenate((initial_mlds[:,2,:].flatten(), retrieved_mlds[:,2,:].flatten()), axis=None))
	#k_max = np.nanmax(np.concatenate((initial_mlds[:,2,:].flatten(), retrieved_mlds[:,2,:].flatten()), axis=None))
	k_min, k_max = 0, 1 # k should always be between these values

	a1 = f1.add_subplot(2,1,1)
	p11 = a1.plot([],[], label='measured', marker='o', linestyle='none')
	p12 = a1.plot([],[], label='retrieved', marker='o', linestyle='none')
	a1.set_ylabel('ln((I/F)_0)')
	a1.set_xlabel('avg latitude (deg)')
	a1.legend(loc='lower left')
	a1.set_xlim(-90, 90)
	a1.set_ylim(ln_IperF0_min, ln_IperF0_max)
	
	a2 = f1.add_subplot(2,1,2)
	p21 = a2.plot([],[], label='measured', marker='o', linestyle='none')
	p22 = a2.plot([],[], label='retrieved', marker='o', linestyle='none')
	a2.set_ylabel('k')
	a2.set_xlabel('avg latitude (deg)')
	a2.legend(loc='upper left')
	a2.set_xlim(-90, 90)
	a2.set_ylim(k_min, k_max)
	...
	def update(i):
		#f1.suptitle(f'lat {lat_ranges[i,0]} to {lat_ranges[i,1]}')
		f1.suptitle(f'wavelength {initial_mlds[0,0,i]:03.2f} (um)') # wavelength grid should be the same for each latitude
		p11[0].set_data(mean_lats, initial_mlds[:,1,i])
		p12[0].set_data(mean_lats, retrieved_mlds[:,1,i])
		p21[0].set_data(mean_lats, initial_mlds[:,2,i])
		p22[0].set_data(mean_lats, retrieved_mlds[:,2,i])
		return

	#ordered_lats = np.sort(lat_ranges[:,0])
	#ordered_idxs = [np.argmin(np.abs(lat_ranges[:,0] - l)) for l in ordered_lats]
	#print(ordered_idxs)
	n_wavs = initial_mlds.shape[2]
	ani = matplotlib.animation.FuncAnimation(f1, update, n_wavs, interval=100)
	outmovname = 'minnaert_params_v_wav.mov'
	print(outfolder)
	plotutils.save_animate_plt(ani, outmovname, outfolder, save_plot=True, animate_plot=show_plot)
	### ------------------------------------------------------------------ ###
	##########################################################################
	...
	print('INFO: Creating synthetic image of minnert fit across planet disk...')
	...
	if kwargs['minnaert.ensemble.regrid_wavelength_to_fits']:
		...
		# try just interpolating instead of subsampling, or make it an option
		import subsample
	...
	n_latitude_overlaps = np.zeros_like(hdul[0].data)
	ln_IperF_0 = np.zeros_like(hdul[0].data)
	k = np.zeros_like(hdul[0].data)
	print(f'INFO: retrieved_radiances_from_minnaert.shape {retrieved_radiances_from_minnaert.shape} n_latitude_overlaps.shape {n_latitude_overlaps.shape}')

	# fill holders with data from each latitude band
	for i, (lat_min, lat_max) in enumerate(lat_ranges):
		...
		lat_slice = np.s_[:, lat_idxs[0], lat_idxs[1]]
		print(f'lat_idxs {lat_idxs}')
		print(f'lat_slice {lat_slice}')
		print(retrieved_mlds[i])
		...
		# we have to assume that our latitude bands can overlap
		# therefore sum the contrubution from each latitude band
		# and keep track of how many latitude bands contributed to a given pixel.
		n_latitude_overlaps[lat_slice] +=  1
		ln_IperF_0[lat_slice] += retrieved_ln_IperF_0[:,None]
		k[lat_slice] += retrieved_k[:, None]

	# divide through by the number of overlapping latitude bands that added values to the same pixel
	# this should work out an average
	ln_IperF_0 /= n_latitude_overlaps
	k /= n_latitude_overlaps

	# observer zenith and solar zenith are the same for neptune (as we are observing from earth)
	fits_u_and_u0 =  np.cos((np.pi/180)*hdul['ZENITH'].data[None,:,:])

	if not kwargs['minnaert.ensemble.cloud_mask.disable']:
		cloud_mask = np.zeros_like(hdul[0].data)
		cm_hdul = fits.open(os.path.join(os.path.dirname(kwargs['minnaert.ensemble.fits']), kwargs['minnaert.ensemble.cloud_mask']))
		cloud_mask[:,:,:] = cm_hdul[0].data[:hdul[0].data.shape[0],:,:]
		hdul[0].data = np.ma.array(hdul[0].data, mask=cloud_mask, fill_value=np.nan)
	...
	#print('INFO: retrieved_radiances_from_minnaert')
	#print(retrieved_radiances_from_minnaert[:,30,30])
	#print(ln_IperF_0[:,30,30])
	#print(k[:,30,30])
	#print(n_latitude_overlaps[:,30,30])

	# create and do the plots, maybe also do a log-scale version?
	nr, nc = 2,4
	f2 = plt.figure(figsize=[_x/2.54 for _x in (12*nc,12*nr)])

	a21 = f2.add_subplot(nr,nc,1)
	ca21 = f2.add_axes([0.12, 0.48, 0.18, 0.02])
	im21 = a21.imshow(np.nanmedian(retrieved_radiances_from_minnaert, axis=0), origin='lower') # dummy data overwritten later
	a21.set_title('log(retrieved radiances from minneart)')
	f2.colorbar(im21, cax=ca21, orientation='horizontal')
	
	a22 = f2.add_subplot(nr,nc,2)
	ca22 = f2.add_axes([0.32, 0.48, 0.18, 0.02])
	im22 = a22.imshow(np.nanmedian(raw_data, axis=0), origin='lower') # dummy data overwritten later
	a22.set_title('log(raw data)')
	f2.colorbar(im22, cax=ca22, orientation='horizontal')
	
	a23 = f2.add_subplot(nr,nc,3)
	ca23 = f2.add_axes([0.52, 0.48, 0.18, 0.02])
	im23 = a23.imshow(np.nanmedian(residual, axis=0), origin='lower') # dummy data overwritten later
	a23.set_title('log(Residual)')
	f2.colorbar(im23, cax=ca23, orientation='horizontal')

	a26 = f2.add_subplot(nr,nc,4)
	ca26 = f2.add_axes([0.72, 0.48, 0.18, 0.02])
	im26 = a26.imshow(np.nanmedian(chi_sq, axis=0), origin='lower') # dummy data overwritten later
	a26.set_title('log(Chi Square)')
	f2.colorbar(im26, cax=ca26, orientation='horizontal')
	...
	p251 = a25.plot(wavgrid, np.nanmean(np.abs(residual), axis=(1,2)), label='mean |residual|', color='tab:green')
	a25_x2 = a25.twinx()
	p252 = a25_x2.plot(wavgrid, np.nanmean(chi_sq, axis=(1,2)), label='mean chi squared', color='tab:purple')
	a25.set_yscale('log')
	...
	def update(i):
		...
		min_clim, max_clim = np.nanmin(raw_data[i]), np.nanmax(raw_data[i])
		...
		im21.set_clim((min_clim, max_clim))
		im22.set_clim((min_clim, max_clim))
		im23.set_clim((min_clim, max_clim))
		im26.set_clim((min_clim, max_clim))
		a24_vline.remove()
	...
	####################################################################################################
	### Create a plot showing the retrieved vs measured radiances across planet disk using LOG SCALE ###
	print('INFO: Creating synthetic image of minnert fit across planet disk in LOG SCALE...')
	...
	retrieved_radiances_from_minnaert_log = np.log(retrieved_radiances_from_minnaert)
	raw_data_log = np.log(raw_data)
	chi_sq_log = np.log(chi_sq)
	residual_log = np.log(residual)
	residual_log_neg = np.log(-1*residual)
	...
	a23 = f2.add_subplot(nr,nc,3)
	ca23 = f2.add_axes([0.52, 0.47, 0.18, 0.02])
	ca231 = f2.add_axes([0.52, 0.49, 0.18, 0.02])
	im23 = a23.imshow(np.nanmedian(residual, axis=0), origin='lower') # dummy data overwritten later
	im231 = a23.imshow(np.nanmedian(residual, axis=0), origin='lower', cmap='gnuplot')
	a23.set_title('log(Residual)')
	cbar23 = f2.colorbar(im23, cax=ca23, orientation='horizontal')
	cbar231 = f2.colorbar(im231, cax=ca231, orientation='horizontal')
	cbar231.set_ticks([])
	...
	def update(i):
		nonlocal a24_vline, a25_vline
		im21.set_data(retrieved_radiances_from_minnaert_log[i,:,:])
		im22.set_data(raw_data_log[i,:,:])
		im23.set_data(residual_log[i,:,:])
		im231.set_data(residual_log_neg[i,:,:])
		im26.set_data(chi_sq_log[i,:,:])
		min_clim, max_clim = np.nanmin(raw_data_log[i]), np.nanmax(raw_data_log[i])
		#im21.set_clim((np.nanmin(retrieved_radiances_from_minnaert_log[i]), np.nanmax(retrieved_radiances_from_minnaert_log[i])))
		#im22.set_clim((np.nanmin(raw_data_log[i]), np.nanmax(raw_data_log[i])))
		#im23.set_clim((np.nanmin(residual_log[i]), np.nanmax(residual_log[i])))
		#im26.set_clim((np.nanmin(chi_sq_log[i]), np.nanmax(chi_sq_log[i])))
		im21.set_clim((min_clim, max_clim))
		im22.set_clim((min_clim, max_clim))
		im23.set_clim((min_clim, max_clim))
		im231.set_clim((min_clim, max_clim))
		im26.set_clim((min_clim, max_clim))
		a24_vline.remove()
		a24_vline = a24.axvline(wavgrid[i], color='red')
		a25_vline.remove()
	...
	ani = matplotlib.animation.FuncAnimation(f2, update, hdul[0].data.shape[0], interval=100)
	outmovname_radiances = 'minnaert_log_radiances.mov'
	plotutils.save_animate_plt(ani, outmovname_radiances, outfolder, save_plot=True, animate_plot=show_plot)
	### -------------------------------------------------------------------------------------------- ###
	####################################################################################################
...
def ensemble_minnaert_parameters(runnames, outfolder=None, show_plot=False, **kwargs):
	"""
	Plots k and IperF0 for apriori and retrieved spectra
	"""
	from fitscube.limb_darkening.minnaert import get_coefficients as get_minnaert_coefficients
	
	ut.pINFO('In "ensemble_minnaert_parameters()"')
	# get number of wavelengths in a *.spx file
	try:
		spxd = nemesis.read.spx(runnames[0])
	except (nemesis.exceptions.NemesisReadError, FileNotFoundError) as e:
		ut.pWARN(f'Could not read *.spx file for runname {runnames[0]}, aborting plot...')
		return
	nwavs = spxd['spec_record'][0].shape[0]
	wavs = spxd['spec_record'][0][:,0]
	
	# create holders for data
	spx_wav_lat_k = np.zeros((len(runnames), nwavs))
	spx_wav_lat_IperF0 = np.zeros((len(runnames), nwavs))
	mre_apr_wav_lat_k = np.zeros((len(runnames), nwavs))
	mre_apr_wav_lat_IperF0 = np.zeros((len(runnames), nwavs))
	mre_ret_wav_lat_k = np.zeros((len(runnames), nwavs))
	mre_ret_wav_lat_IperF0 = np.zeros((len(runnames), nwavs))
	
	# sort runs by latitude
	lat_arr = np.zeros((len(runnames),))
	for i, runname in enumerate(runnames):
		try:
			spxd = nemesis.read.spx(runname)
		except (nemesis.exceptions.NemesisReadError, FileNotFoundError) as e:
			ut.pWARN(f'Could not read *.spx file for runname {runname}, aborting plot...')
			return
		lat_arr[i] = spxd['latitude']
	lat_order_idx = np.argsort(lat_arr)
	
	# loop over runs in order of latitude and fill data holders
	for _i, _j in enumerate(lat_order_idx):
		runname =runnames[_j]
		#print(f'DEBUG: runname {runname}')
		try:
			spxd = nemesis.read.spx(runname)
		except (nemesis.exceptions.NemesisReadError, FileNotFoundError) as e:
			ut.pWARN(f'Could not read *.spx file for runname {runname}, aborting plot...')
			return
		u0s = np.cos((np.pi/180)*np.array([fov_geom[0,2] for fov_geom in spxd['fov_averaging_record']]))
		us = np.cos((np.pi/180)*np.array([fov_geom[0,2] for fov_geom in spxd['fov_averaging_record']]))
		IperFs = np.array([sr[:,1] for sr in spxd['spec_record']])
		for _k in range(nwavs):
			IperF0, k, residual, rank, s, log_u0u, log_uIperF, valid_idxs, cov_mat = get_minnaert_coefficients(u0s, us, IperFs[:,_k], two_points_ok_flag=True)
			spx_wav_lat_k[_i,_k] = k
			spx_wav_lat_IperF0[_i, _k] = IperF0
		#print(f'DEBUG: u0s {u0s}')
		#print(f'DEBUG: us {us}')
		#print(f'DEBUG: IperFs {IperFs}')
		#print(f'DEBUG: spx_wav_lat_k')
		#for _ii in range(lat_order_idx.size): print(f'{spx_wav_lat_k[_ii]}')
		...
		try:
			...
			ut.pWARN(f'Could not read *.mre file for runname {runname}, setting retrieved and apriori minnaert parameters to NAN...')
			mre_nan_flag=True
		if mre_nan_flag:
			mre_apr_wav_lat_k[_i,:] = np.full((nwavs,), fill_value=np.nan)
			mre_apr_wav_lat_IperF0[_i,:] = np.full((nwavs,), fill_value=np.nan)  
			mre_ret_wav_lat_k[_i,:] = np.full((nwavs,), fill_value=np.nan)
			mre_ret_wav_lat_IperF0[_i,:] = np.full((nwavs,), fill_value=np.nan)  
		else:
			IperFs_apr = mred['radiance_meas']
			IperFs_ret = mred['radiance_retr']
			for _k in range(nwavs):
				IperF0_apr, k_apr, residual, rank, s, log_u0u, log_uIperF, valid_idxs, cov_mat = get_minnaert_coefficients(u0s, us, IperFs_apr[:,_k], two_points_ok_flag=True)
				mre_apr_wav_lat_k[_i,_k] = k_apr
				mre_apr_wav_lat_IperF0[_i, _k] = IperF0_apr
				IperF0_ret, k_ret, residual, rank, s, log_u0u, log_uIperF, valid_idxs, cov_mat = get_minnaert_coefficients(u0s, us, IperFs_ret[:,_k], two_points_ok_flag=True)
				mre_ret_wav_lat_k[_i,_k] = k_ret
				mre_ret_wav_lat_IperF0[_i, _k] = IperF0_ret
	
	# is data the right way around?
	...
	extent = (wavs[0], wavs[-1], lat_arr[lat_order_idx[0]], lat_arr[lat_order_idx[-1]])
	imshow_kw = {'extent':extent, 'origin':'lower', 'aspect':'auto', 'cmap':cmap}
	
	def plot_wav_lat(fig, ax, data, title='', cbar_label='', **imshow_kw):
		im = ax.imshow(data, **imshow_kw)
		ax.set_title(title)
		cb = f1.colorbar(im, ax=ax, extend='both')
		cb.set_label(cbar_label)
	
	
	plot_wav_lat(f1, a1[0,0], spx_wav_lat_k, cbar_label='spx k', **imshow_kw, vmin=0,vmax=1)
	
	plot_wav_lat(f1, a1[0,1], mre_apr_wav_lat_k, cbar_label='mre apr k', **imshow_kw, vmin=0,vmax=1)
	
	plot_wav_lat(f1, a1[0,2], mre_ret_wav_lat_k, cbar_label='mre ret k', **imshow_kw, vmin=0,vmax=1)
	
	plot_wav_lat(f1, a1[1,0], spx_wav_lat_IperF0, cbar_label=r'spx $(I/F)_0$ ($W \, cm^{-2} \, sr^{-1} \, um^{-1}$)', **imshow_kw)
	
	plot_wav_lat(f1, a1[1,1], mre_apr_wav_lat_IperF0, cbar_label=r'mre apr $(I/F)_0$ ($\mu W \, cm^{-2} \, sr^{-1} \, um^{-1}$)', **imshow_kw)
	
	plot_wav_lat(f1, a1[1,2], mre_ret_wav_lat_IperF0, cbar_label=r'mre ret $(I/F)_0$ ($\mu W \, cm^{-2} \, sr^{-1} \, um^{-1}$)', **imshow_kw)
	
	f1.tight_layout
	save_show_plt(f1, "ensemble_minnaert_parameters.png", outfolder=outfolder, show_plot=show_plot)
	return([f1])
...
def ensemble_params_v_lat(runnames, outfolder=None, show_plot=False, **kwargs):
	...
	"""
	(indep_lbls, dep_lbls, re_dep_arr, re_dep_err, ap_dep_arr, ap_dep_err, 
	sorted_lat_arr, nindep_each_param, indep_arr, 
	ap_params, re_params) = get_params_vs_lat(runnames)

	print(indep_lbls, dep_lbls, re_dep_arr, re_dep_err, ap_dep_arr, ap_dep_err, 
			sorted_lat_arr, nindep_each_param, indep_arr)
	...
	for i, indep_lbl in enumerate(indep_lbls):
		for j in range(nindep_each_param[i]):
			...
			print(f'INFO: Saving frame {j} to file "{savefig_name}"')
		...
		# if we have parameters for every datapoint
		if all([_x is not None for _x in ap_params[i]]) and all([_x is not None for _x in re_params[i]]):
			param_names = ap_params[i][0].keys()
			for param_name in param_names:
				print(f'INFO: {dep_lbls[i]} {param_name} v lat')
				f2 = plt.figure()
				a2 = f2.subplots(1,1,squeeze=False)
				ap_p_val = [_d[param_name][0] for _d in ap_params[i]]
				ap_p_err = [_d[param_name][1] for _d in ap_params[i]]
				re_p_val = [_d[param_name][0] for _d in re_params[i]]
				re_p_err = [_d[param_name][1] for _d in re_params[i]]
				a2[0,0].plot(sorted_lat_arr, ap_p_val, label='apriori value', color='tab:blue')
				a2[0,0].plot(sorted_lat_arr, re_p_val, label='retrieved value', color='tab:orange')
				a2[0,0].fill_between(sorted_lat_arr, 
										[_x - _y for (_x, _y) in zip(ap_p_val, ap_p_err)],
										[_x + _y for (_x,_y) in zip(ap_p_val, ap_p_err)],
										color='tab:blue', label='apriori error', alpha=0.5)
				a2[0,0].fill_between(sorted_lat_arr,
										[_x - _y for (_x, _y) in zip(re_p_val, re_p_err)],
										[_x + _y for (_x,_y) in zip(re_p_val, re_p_err)],
										color='tab:orange', label='retrieved error', alpha=0.5)
				a2[0,0].set_xlabel('Latitude (deg North)')
				a2[0,0].set_ylabel(f'{param_name}')
				a2[0,0].set_title(dep_lbls[i])

				figure_name = f'{dep_lbls[i]}_{param_name}_v_latitude.png'
				
				f2.savefig(os.path.join(outfolder, figure_name))
				plt.close(f2)
...
def ensemble_params_v_lon(runnames, outfolder=None, show_plot=False, **kwargs):
	...
	"""
	(indep_lbls, dep_lbls, re_dep_arr, re_dep_err, ap_dep_arr, ap_dep_err, 
	sorted_lon_arr, nindep_each_param, indep_arr, 
	ap_params, re_params) = get_params_vs_lat(runnames, prop='lon')
...
def get_params_vs_lat(runnames, prop='lat'):
	"""
	For a set of runs, returns data in an arrangement where parameters vs 
	latitude is easy to plot

	ARGUMENTS:
		runnames
			<str,array> A list of runnames to operate on
		prop
			<str> The property to get params against, originally hard-coded as 
			'lat' (latitude) but made more adaptable.

	RETURNS:
		indep_lbls [nparam]
			<str,array> The labels of the independent variables for each 
			parameter
		dep_lbls [nparams]
			<str,array> The labels of the dependent variable for each parameter
		re_dep_arr [nparams,[nruns,nvar[i]]]
			<list,<float,array>> Retrieved values for each parameter. Consists 
			of 'nparam' numpy arrays each of which has a size of 'nruns' by 
			'nvar[i]' where 'nruns' are the number of OPENABLE runs, and 
			'nvar[i]' is the number of values in the i^th parameter
		re_dep_err [nparams,[nruns,nvar[i]]]
			<list,<float,array>> Error on the retrieved values for each 
			parameter. Arranged in the same way as 're_dep_arr'
		ap_dep_arr [nparams,[nruns,nvar[i]]]
			<list,<float,array>> Apriori values for each parameter. Arranged in
			 the same way as 're_dep_arr'
		ap_dep_err [nparams,[nruns,nvar[i]]]
			<list,<float,array> Error on the apriori values for each parameter.
			Arranged in the same way as 're_dep_arr'
		sorted_lat_arr [nruns]
			<float,array> The latitude of each OPENABLE run
		nindep_each_param [nparam]
			<int,array> The number of values (specifically independent values, 
			but this is the same as the dependent values) for each parameter.
		indep_arr [nparam,nvar[i]]
			<list,<float,array>> The values of the independent variable for 
			each parameter.
		ap_params
			A list of apriori parameters (as a dictionary), or None if not
			needed (only parameterisations have params). Value, error pairs
		re_params
			A list of retrieved parameters (as a dictionary) or None if not
			needed (only parmeterisations have params). Value, error pairs.

	NOTES:
		nruns
			The number of OPENABLE runs, by 'openable' we mean runs for which 
			nemesis.read.mre() succeeded with no errors
		nvar[i]
			The number of (x,y) pairs needed to describe the i^th parameter. 
			E.g. a volume mixing ratio may have 20 pressure levels where it is 
			defined.

	EXAMPLE:
		(indep_lbls, dep_lbls, re_dep_arr, re_dep_err, ap_dep_arr, ap_dep_err, 
		sorted_lat_arr, nindep_each_param, 
		indep_arr) = get_params_vs_lat(runnames)
	"""
	xdata = []
	re_ydata = {}
	ap_ydata = {}
	...
	for i, runname in enumerate(runnames):
		...
		if runname_var_info[i] is None:
			continue

	# filter runnames by runs that have finished and not been interrupted
	runnames = [runname for i, runname in enumerate(runnames) if runname_var_info[i] is not None]
	lat_arr = np.array([l for i, l in enumerate(lat_arr) if runname_var_info[i] is not None])
	# this one has to go last as the others rely upon it for filtering
	runname_var_info = [vi for i, vi in enumerate(runname_var_info) if vi is not None]
	...
	re_params = [[None]*len(runnames) for _i,_x in enumerate(vid_hashes)]
	ap_params = [[None]*len(runnames) for _i,_x in enumerate(vid_hashes)]
	...
	for i, runname in enumerate(runnames):
		for j, vid_hash in enumerate(vid_hashes):
			...
			re_params[j][i] = runname_var_info[i][vid_hash][vdat_type].get('parameters', None)
			vdat_type = 'apriori'
			...
			ap_params[j][i] = runname_var_info[i][vid_hash][vdat_type].get('parameters', None)
			print(i, j, vid_hash, vdat_type)
			print(runname_var_info[i][vid_hash][vdat_type])
	...
	return(	indep_lbls, dep_lbls, re_dep_arr, re_dep_err, ap_dep_arr, 
			ap_dep_err, sorted_lat_arr, nindep_each_param, indep_arr, 
			ap_params, re_params)
...
def varident_to_labels_and_profiles(runname, varident, varparams, state_vector, state_vector_err):
	...
	ARGUMENTS:
		...
		state_vector [nv]
			<float, array> The section of the state vector that corresponds to 
			the variable under consideration
		state_vector_err [nv]
			<float, array> The section of the error on the state vector that 
			corresponds to the variable under consideration

	RETURNS:
		A dictionay with the following keys:
			independent_var
				<float, array> The independent variable that the variable under
				consideration is computed with respect to (e.g. pressure grid)
			dependent_var
				<float, array> The value of the variable under consideration at
				each point of the independent variable
			dependent_err
	...
	var_info = var_repr_func(varparams, state_vector, state_vector_err, height_grid, 
							pressure_grid, wavelength_grid, runname)
...
#
# 
plot_arg_choice_groups = 	{	'all':[mre_spectral, mre_parameter, chi_sq, improvement_factor, limb_darkening, mre_diff, spx], # this shoud plot everything it can
								'mre':[mre_spectral, mre_parameter, mre_diff],
								'itr':[chi_sq],
								'mre_spectral':[mre_spectral, mre_diff],
...
plot_arg_choice_help = """\
	...
	Plot types are:
		...
		mre_diff
			Plots the differences between geomoetries in an mre file
		spx
			Plots the information contained within an spx file\
	"""
...
ensemble_arg_choice_groups = 	{	'all':[ensemble_improvement_factor, ensemble_params_v_lat, ensemble_params_v_lon, ensemble_chi_phi_v_lat, ensemble_chi_phi_v_lon],
									...
									'latitude':[ensemble_params_v_lat, ensemble_chi_phi_v_lat],
									'minnaert':[ensemble_minnaert_limb_darkening, ensemble_minnaert_parameters]
								}
ensemble_arg_choice_help="""\
	...
	Plot types are:
		...
		ensemble_minnaert_parameters
			Plots color map plots of k and IperF0 for apriori and retrieved spectra
		\
	"""
...
def parse_args(argv):
	...
	parser.add_argument('--minnaert.ensemble.cloud_mask', type=str, help='Will look for a "./mask.fits" file relative to "--minnaert.ensemble.fits" if leading character is a "." or will look for an absolute file path if leading character is a "/"', default='./mask.fits')
	parser.add_argument('--minnaert.ensemble.cloud_mask.disable', action='store_true', help='If present, will not use a cloud mask for radiance plots.')

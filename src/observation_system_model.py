...
def plot_ga(geo_array, mutator = lambda x:x, title='', data_units='', axes_units=''):
	if type(axes_units) is not tuple:
		axes_units = tuple(axes_units for _ in range(geo_array.data.ndim))
	plt.clf()
	f = plt.gcf()
	f.suptitle(title)
	geo_array.plot(f, data_mutator=lambda x : np.log(np.abs(x)), data_units=data_units, axes_units=axes_units)
	plt.show()
...
class PSFModel:
	...
	def ao_corrections_to_phase_psd(self, phase_psd, ao_phase_psd, f_ao):
		...
		f_mesh = axes_to_mesh(phase_psd.axes)
		f_mag = np.sqrt(np.sum(f_mesh**2, axis=0))
		f_ao_correct = f_mag <= f_ao
	...
	def __call__(self,
		...
		# TODO: Do stuff here to adjust for wavelength of light!
		self.shape = shape
	...
	def at(self, scale, wavelength, plots=True):
		...
		if plots: plot_ga(result, lambda x: np.log(x), f'psf at {wavelength=}', 'arbitrary units', 'radians')
		
		return result
		
	
	
def diffraction_limited_otf(shape, expansion_factor, supersample_factor, ocs, pf_scale):
